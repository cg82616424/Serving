...
def preprocess_img(img_list):
    ...
    Args:
        img_list: list for img file path
    Return:
        image content binary list after preprocess
    """
    preprocess = Sequential([File2Image(), Resize(
        (512, 512))])
    result_list = []
    for img in img_list:
...
def benckmark_worker(idx, resource):
    ...
    Args:
        idx, worker idx ,use idx to select backend unet service
        resource , unet serving endpoint dict 
    Return:
        latency
    TODO:
        http benckmarks
    ...
    client_handler.connect([resource["endpoint"][idx % len(resource["endpoint"])]])
    start = time.time()
    ...
    for i in range(turns):
        if args.batch_size >= 1:
            ...
            result = client_handler.predict(feed={"image":img_list[bi]}, fetch=["output"])
...
if __name__ == '__main__':
    ...
    result = multi_thread_runner.run(
        benckmark_worker, args.thread, {"endpoint": endpoint_list,
                                   "turns": turns, "img_list":img_content_list})
    end = time.time()
